  ...
  return (
    <div className="mx-auto flex min-h-screen max-w-xl flex-col px-5 py-10">
      ...
      <div className="text-center">
        ...
        <h2 className="mt-2 text-2xl font-bold text-slate-900 dark:text-slate-100">Liderlik lövhəsi</h2>
        <p className="mt-1 text-sm text-slate-500 dark:text-slate-400">Bu cihazda saxlanan bütün nəticələr üzrə sıralama</p>
      </div>

      {profile && (
        <div className="mt-4 rounded-xl bg-indigo-50 px-4 py-3 text-center text-sm font-semibold text-indigo-700 ring-1 ring-indigo-200 dark:bg-indigo-500/10 dark:text-indigo-400 dark:ring-indigo-500/30">
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        <select
          ...
          className="rounded-lg bg-white px-3 py-2 text-sm text-slate-700 shadow-sm ring-1 ring-slate-200 outline-none dark:bg-slate-800 dark:text-slate-300 dark:ring-slate-700"
        >
      ...
      <div className="mt-5 flex flex-col gap-1.5">
        {ranked.length === 0 && (
          <p className="mt-6 text-center text-sm text-slate-400 dark:text-slate-500">Bu filtr üzrə hələ nəticə yoxdur.</p>
        )}
        {ranked.map((h, i) => {
          const rank = i + 1
          ...
          return (
            <div
              ...
              className={`flex items-center gap-3 rounded-lg px-3 py-2.5 text-sm shadow-sm ring-1 ${
                mine ? 'bg-indigo-50 ring-indigo-300 dark:bg-indigo-500/10 dark:ring-indigo-500/40' : 'bg-white ring-slate-200 dark:bg-slate-800 dark:ring-slate-700'
              }`}
            >
              <span className={`w-7 shrink-0 text-center font-semibold ${mine ? 'text-indigo-700 dark:text-indigo-400' : 'text-slate-400 dark:text-slate-500'}`}>
                {medal ?? rank}
              </span>
              <div className="min-w-0 flex-1 text-left">
                <div className={`truncate font-semibold ${mine ? 'text-indigo-900 dark:text-indigo-300' : 'text-slate-800 dark:text-slate-200'}`}>
                  {h.name} {mine && <span className="text-xs font-normal text-indigo-500 dark:text-indigo-400">(Sən)</span>}
                </div>
                <div className="truncate text-xs text-slate-400 dark:text-slate-500">
                  {subject.emoji} {subject.name} · {difficultyMeta[h.difficulty].label}
                </div>
              </div>
              <span className={`shrink-0 font-bold ${mine ? 'text-indigo-700 dark:text-indigo-400' : 'text-slate-900 dark:text-slate-100'}`}>{h.score}</span>
            </div>
          )
        })}
      </div>
