...
export default function Home({ onSelectSubject, bestOverall }: Props) {
  return (
    <div className="mx-auto flex min-h-screen max-w-3xl flex-col items-center px-5 py-10 text-center">
      ...
      <h1 className="text-3xl font-extrabold text-white sm:text-4xl">Bilik Ustası</h1>
      ...
      {bestOverall > 0 && (
        <div className="mt-4 rounded-full bg-white/5 px-4 py-1.5 text-xs font-medium text-amber-300 ring-1 ring-white/10">
      ...
      <div className="mt-8 grid w-full grid-cols-2 gap-3 sm:grid-cols-3">
        {subjects.map((s) => (
          <button
            ...
            className={`group flex flex-col items-center justify-center gap-2 rounded-2xl bg-gradient-to-br ${s.gradient} p-5 shadow-lg transition-transform duration-150 hover:scale-105 active:scale-95`}
          >
      ...
      <p className="mt-10 text-xs text-slate-500">Riyaziyyat · Coğrafiya · Tarix · İngilis dili · Azərbaycan dili · Fizika · Kimya</p>
    </div>
  )
}
